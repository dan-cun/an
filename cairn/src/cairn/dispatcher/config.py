...
class WorkerConfig(BaseModel):
    ...
    def validate_env(self) -> "WorkerConfig":
        ...
        if self.type == "pi":
            ...
            if provider_api and provider_api not in PI_PROVIDER_APIS:
                allowed = ", ".join(sorted(PI_PROVIDER_APIS))
                raise ValueError(f"worker {self.name} env PI_PROVIDER_API must be one of: {allowed}")
        if self.type == "mock":
...
class DispatchConfig(BaseModel):
    ...
    def load(cls, path: Path) -> "DispatchConfig":
        ...
        data = resolve_env_references(data)
...
def resolve_env_references(value: Any) -> Any:
    """Resolve ${NAME} references in YAML values without supporting shell expansion.

    Dispatcher configurations often need API keys inside worker environment maps. Keeping
    the secret in the process environment avoids committing it to dispatch.yaml while the
    deliberately small ${NAME} syntax remains predictable across Docker and local mode.
    """
    ...
    def replace(match: re.Match[str]) -> str:
        ...
        if name not in os.environ:
            raise ValueError(f"environment variable {name} referenced by dispatcher config is not set")
