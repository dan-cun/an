...
import { AlertOutlined, AuditOutlined, DashboardOutlined, DatabaseOutlined, FileTextOutlined, MenuFoldOutlined, MenuUnfoldOutlined, RadarChartOutlined, RobotOutlined, ToolOutlined, TeamOutlined } from '@ant-design/icons'
...
const routes = { '/cockpit': ['安全运营驾驶舱', <RadarChartOutlined />], '/dashboard': ['安全态势总览', <DashboardOutlined />], '/workbench': ['任务编排', <TeamOutlined />], '/audit': ['审计与拦截', <AuditOutlined />], '/experiences': ['经验学习库', <DatabaseOutlined />], '/incident-response': ['应急响应', <AlertOutlined />], '/models': ['模型与用量', <RobotOutlined />], '/prompts': ['Prompt 目录', <FileTextOutlined />], '/mcp': ['MCP 工具清单', <ToolOutlined />] }
...
export function FeatureApp() {
  ...
  const cockpitHost = typeof window !== 'undefined' && (window.location.port === '5174' || import.meta.env.MODE === 'cockpit' || import.meta.env.VITE_COCKPIT_ONLY === 'true')
  const active = cockpitHost && location.pathname.startsWith('/cockpit') ? '/cockpit' : location.pathname.startsWith('/audit') ? '/audit' : location.pathname.startsWith('/experiences') ? '/experiences' : location.pathname.startsWith('/incident-response') ? '/incident-response' : location.pathname.startsWith('/models') ? '/models' : location.pathname.startsWith('/prompts') ? '/prompts' : location.pathname.startsWith('/mcp') ? '/mcp' : location.pathname.startsWith('/workbench') ? '/workbench' : '/dashboard'
  ...
  const items = useMemo(() => [
    { type: 'group', label: '安全运营', children: [
      ...(cockpitHost ? [{ key: '/cockpit', icon: <RadarChartOutlined />, label: '驾驶舱' }] : []),
  ...
  return <Layout className={`feature-shell ${collapsed ? 'is-collapsed' : ''}`}>
    ...
    <Sider className="feature-sidebar" width={236} collapsedWidth={72} collapsed={collapsed} trigger={null}>
      <button className="sidebar-title" type="button" onClick={() => navigate(cockpitHost ? '/cockpit' : '/dashboard')}><span><b>安全智能体平台</b><small>任务编排与审计</small></span></button>
    ...
    <Layout className="feature-main">
      ...
      <Content className="feature-content"><Routes><Route path="/cockpit" element={cockpitHost ? <CockpitPage /> : <Navigate to="/dashboard" replace />} /><Route path="/dashboard" element={<DashboardPage />} /><Route path="/workbench" element={<WorkbenchPage />} /><Route path="/audit" element={<AuditReplayPage />} /><Route path="/audit/:runId" element={<AuditReplayPage />} /><Route path="/experiences" element={<ExperienceLibraryPage />} /><Route path="/incident-response" element={<IncidentResponsePage />} /><Route path="/models" element={<ModelUsagePage />} /><Route path="/prompts" element={<PromptCatalogPage />} /><Route path="/mcp" element={<McpRegistryPage />} /><Route path="*" element={<Navigate to={cockpitHost ? '/cockpit' : '/dashboard'} replace />} /></Routes></Content>
    </Layout>
  </Layout>
}
