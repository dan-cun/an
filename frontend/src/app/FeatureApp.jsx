...
export function FeatureApp() {
  ...
  const active = location.pathname.startsWith('/cockpit') ? '/cockpit' : location.pathname.startsWith('/audit') ? '/audit' : location.pathname.startsWith('/experiences') ? '/experiences' : location.pathname.startsWith('/incident-response') ? '/incident-response' : location.pathname.startsWith('/models') ? '/models' : location.pathname.startsWith('/prompts') ? '/prompts' : location.pathname.startsWith('/mcp') ? '/mcp' : location.pathname.startsWith('/workbench') ? '/workbench' : '/dashboard'
  ...
  return <Layout className={`feature-shell ${collapsed ? 'is-collapsed' : ''}`}>
    ...
    <Layout className="feature-main">
      ...
      <Content className="feature-content"><Routes><Route path="/cockpit" element={<CockpitPage />} /><Route path="/dashboard" element={<DashboardPage />} /><Route path="/workbench" element={<WorkbenchPage />} /><Route path="/audit" element={<AuditReplayPage />} /><Route path="/audit/:runId" element={<AuditReplayPage />} /><Route path="/experiences" element={<ExperienceLibraryPage />} /><Route path="/incident-response" element={<IncidentResponsePage />} /><Route path="/models" element={<ModelUsagePage />} /><Route path="/prompts" element={<PromptCatalogPage />} /><Route path="/mcp" element={<McpRegistryPage />} /><Route path="*" element={<Navigate to="/cockpit" replace />} /></Routes></Content>
    </Layout>
  </Layout>
}
