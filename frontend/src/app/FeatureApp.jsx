...
const routes = { '/dashboard': ['安全运营驾驶舱', <DashboardOutlined />], '/cockpit': ['安全运营驾驶舱', <DashboardOutlined />], '/workbench': ['任务编排', <TeamOutlined />], '/audit': ['审计与拦截', <AuditOutlined />], '/experiences': ['经验学习库', <DatabaseOutlined />], '/incident-response': ['应急响应', <AlertOutlined />], '/models': ['模型与用量', <RobotOutlined />], '/prompts': ['Prompt 目录', <FileTextOutlined />], '/mcp': ['MCP 工具清单', <ToolOutlined />] }
...
export function FeatureApp() {
  ...
  const items = useMemo(() => [
    { type: 'group', label: '安全运营', children: [
      { key: '/dashboard', icon: <DashboardOutlined />, label: '安全运营驾驶舱' },
