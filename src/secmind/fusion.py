"""Deterministic routing for the unified security workbench.

The router intentionally runs before any model call.  It uses explainable keyword,
extension and magic-byte signals and returns a stable payload suitable for the UI.
"""
...
from secmind.schemas import AgentState, Scenario
...
EXTENSIONS = {
    ...
    "penetration": {".pcap", ".pcapng", ".nessus", ".nmap", ".txt"},
}
...
def classify_task(
    ...
    # Binary magic bytes are represented by the filename when the ingestor has
    # already produced an InputArtifact; avoid reading untrusted paths here.
...
def routing_for_state(state: AgentState) -> dict[str, Any]:
    return classify_task(state.task.objective, state.input_artifacts)
