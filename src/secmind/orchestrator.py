...
class SecMindOrchestrator:
    ...
    async def _retrieve_context(self, value: GraphState) -> GraphState:
        ...
        state.decisions.append(
            DecisionRecord(
                ...
                rationale_summary="当前只读静态审计不需要外部知识上下文。",
    ...
    async def _execute(self, value: GraphState) -> GraphState:
        ...
        result = await self.broker.invoke(
            ...
            ToolContext(
                ...
                module_base_url=(self.settings.reverse_base_url if state.module_route == "reverse" else self.settings.cairn_base_url if state.module_route == "penetration" else None),
            ),
        )
    ...
    async def _memory_commit(self, value: GraphState) -> GraphState:
        ...
        payload = {
            "accepted": accepted,
            "reason": "已完成且通过验证的任务可进入情景记忆候选。"
            if accepted
            else "只有已完成且通过验证的任务才能进入长期记忆。",
        }
