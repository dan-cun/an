...
Qdrant 在 Compose 中预留给知识库和经过 Verifier 门禁的情景记忆。当前代码审计基线不依赖外部知识，避免在证据不充分时引入检索结论。

## 模型策略

确定性路由、预算、权限、证据校验和失败收敛不交给模型。非演示模式下 Planner 使用兼容接口结构化输出；模型失败时降级到静态只读审计计划。任何模型输出都必须再次通过计划 Schema 和工具白名单验证。
